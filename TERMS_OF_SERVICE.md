# Termos de Serviço - Assistente Financeiro

**Data de vigência**: 26 de Setembro de 2025

## 1. Aceitação dos Termos

Ao baixar, instalar ou usar o aplicativo Assistente Financeiro ("App", "Serviço"), você concorda em cumprir e estar vinculado a estes Termos de Serviço ("Termos"). Se você não concorda com estes termos, não use o App.

## 2. Descrição do Serviço

### 2.1 Funcionalidades
O Assistente Financeiro é um aplicativo móvel que oferece:
- Gerenciamento de gastos pessoais
- Categorização automática de transações
- Relatórios e análises financeiras
- Chat com assistente de IA para orientação financeira
- Sincronização de dados entre dispositivos

### 2.2 Natureza do Serviço
- O App é uma ferramenta de organização financeira pessoal
- Não oferecemos consultoria financeira profissional
- Não somos uma instituição financeira
- Não processamos pagamentos ou transações bancárias

## 3. Elegibilidade e Conta

### 3.1 Idade Mínima
- Você deve ter pelo menos 18 anos para usar este App
- Ao usar o App, você declara ter a idade mínima exigida

### 3.2 Criação de Conta
- Você deve fornecer informações precisas e atualizadas
- É responsável por manter a segurança da sua conta
- Deve notificar imediatamente sobre uso não autorizado

### 3.3 Uma Conta por Pessoa
- Cada usuário pode ter apenas uma conta
- Contas duplicadas podem ser suspensas ou excluídas

## 4. Uso Aceitável

### 4.1 Uso Permitido
Você pode usar o App para:
- Gerenciar suas finanças pessoais
- Registrar transações e gastos
- Gerar relatórios para uso pessoal
- Interagir com o assistente de IA

### 4.2 Uso Proibido
Você NÃO pode:
- Usar o App para atividades ilegais
- Tentar hackear ou comprometer a segurança
- Compartilhar sua conta com terceiros
- Usar bots ou automação não autorizada
- Inserir dados falsos ou enganosos
- Violar direitos de propriedade intelectual

### 4.3 Consequências
Violações podem resultar em:
- Suspensão temporária da conta
- Exclusão permanente da conta
- Ação legal quando aplicável

## 5. Dados e Privacidade

### 5.1 Seus Dados
- Você mantém a propriedade dos seus dados financeiros
- Somos apenas processadores dos dados que você insere
- Consulte nossa Política de Privacidade para detalhes

### 5.2 Responsabilidade pelos Dados
- Você é responsável pela precisão dos dados inseridos
- Não somos responsáveis por decisões baseadas em dados incorretos
- Recomendamos backup regular dos seus dados

## 6. Propriedade Intelectual

### 6.1 Nossos Direitos
- O App e todo seu conteúdo são de nossa propriedade
- Marcas, logos e design são protegidos por direitos autorais
- Você recebe apenas uma licença limitada de uso

### 6.2 Licença de Uso
Concedemos uma licença:
- Não exclusiva e intransferível
- Para uso pessoal apenas
- Revogável a qualquer momento
- Limitada aos termos deste acordo

## 7. Limitações de Responsabilidade

### 7.1 Isenção de Garantias
O App é fornecido "como está", sem garantias de:
- Funcionamento ininterrupto
- Ausência de erros ou bugs
- Adequação a propósitos específicos
- Resultados financeiros específicos

### 7.2 Limitação de Danos
Não somos responsáveis por:
- Perdas financeiras baseadas no uso do App
- Danos indiretos ou consequenciais
- Perda de dados devido a falhas técnicas
- Decisões financeiras tomadas com base no App

### 7.3 Valor Máximo
Nossa responsabilidade total não excederá o valor pago pelo App nos últimos 12 meses.

## 8. Consultoria Financeira

### 8.1 Não Somos Consultores
- O App fornece ferramentas, não conselhos profissionais
- Não somos licenciados como consultores financeiros
- Não fazemos recomendações de investimento

### 8.2 Assistente de IA
- O chat com IA fornece informações gerais apenas
- Não substitui consultoria profissional
- Sempre consulte um profissional para decisões importantes

### 8.3 Sua Responsabilidade
- Você é responsável por suas decisões financeiras
- Recomendamos consultar profissionais qualificados
- Use o App como ferramenta complementar apenas

## 9. Pagamentos e Reembolsos

### 9.1 Versão Gratuita
- A versão atual do App é gratuita
- Recursos premium podem ser adicionados no futuro

### 9.2 Futuras Funcionalidades Pagas
Se implementarmos recursos pagos:
- Preços serão claramente informados
- Cobrança será feita através das lojas de app
- Política de reembolso seguirá as regras da loja

## 10. Modificações do Serviço

### 10.1 Atualizações
- Podemos atualizar o App regularmente
- Algumas atualizações podem ser obrigatórias
- Recursos podem ser adicionados ou removidos

### 10.2 Descontinuação
- Podemos descontinuar o App com aviso de 30 dias
- Você poderá exportar seus dados antes da descontinuação
- Não há garantia de funcionamento perpétuo

## 11. Modificações dos Termos

### 11.1 Direito de Alteração
- Podemos modificar estes termos a qualquer momento
- Mudanças significativas serão notificadas no App
- Uso continuado constitui aceitação dos novos termos

### 11.2 Recusa de Novos Termos
- Se não concordar com novos termos, pare de usar o App
- Você pode excluir sua conta a qualquer momento

## 12. Rescisão

### 12.1 Rescisão por Você
- Você pode parar de usar o App a qualquer momento
- Pode solicitar exclusão da conta através do App
- Dados serão excluídos conforme Política de Privacidade

### 12.2 Rescisão por Nós
Podemos suspender ou encerrar sua conta por:
- Violação destes termos
- Uso fraudulento ou abusivo
- Inatividade prolongada (mais de 2 anos)
- Descontinuação do serviço

## 13. Lei Aplicável e Jurisdição

### 13.1 Lei Brasileira
- Estes termos são regidos pela lei brasileira
- Disputas serão resolvidas no Brasil
- Idioma oficial dos termos é o português

### 13.2 Resolução de Disputas
1. **Negociação**: Tentativa de resolução amigável
2. **Mediação**: Através de câmara de mediação
3. **Arbitragem**: Se necessário, conforme lei brasileira
4. **Foro**: Comarca de São Paulo, SP, Brasil

## 14. Disposições Gerais

### 14.1 Integralidade
- Estes termos constituem o acordo completo
- Substituem acordos anteriores sobre o mesmo assunto

### 14.2 Severabilidade
- Se alguma cláusula for inválida, as demais permanecem válidas
- Cláusulas inválidas serão interpretadas da forma mais próxima possível

### 14.3 Renúncia
- Não exercer um direito não constitui renúncia
- Renúncias devem ser expressas e por escrito

### 14.4 Cessão
- Você não pode transferir seus direitos sob estes termos
- Podemos transferir nossos direitos mediante notificação

## 15. Contato

### 15.1 Dúvidas sobre os Termos
Para questões sobre estes termos:

- **Email**: contato@assistentefinanceiro.app
- **Endereço**: Rua das Flores, 123 - São Paulo, SP - Brasil
- **Telefone**: +55 (11) 99999-9999

### 15.2 Suporte Técnico
Para problemas técnicos ou suporte:
- Use a função "Contato" dentro do App
- Email: suporte@assistentefinanceiro.app

### 15.3 Tempo de Resposta
- Responderemos em até 48 horas (dias úteis)
- Questões complexas podem levar até 5 dias úteis

---

**Última atualização**: 26 de Setembro de 2025

**Versão**: 1.0

---

## Resumo dos Pontos Principais

✅ **O que você pode fazer**: Usar o App para gerenciar suas finanças pessoais

❌ **O que você não pode fazer**: Usar para atividades ilegais, compartilhar conta, inserir dados falsos

⚠️ **Importante saber**: 
- Não somos consultores financeiros
- Você é responsável por suas decisões financeiras
- O App é fornecido "como está"
- Seus dados são protegidos conforme nossa Política de Privacidade

📞 **Dúvidas**: Entre em contato através de contato@assistentefinanceiro.app

---

*Estes termos foram elaborados para ser justos e transparentes. Se você tiver dúvidas, entre em contato conosco.*


